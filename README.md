...
`projects archive` is reversible with `--restore`. `projects delete` permanently deletes the board, its tasks, notifications, and memberships, so it refuses to run without `--yes`.
